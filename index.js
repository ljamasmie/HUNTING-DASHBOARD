...
async function handleHealth(env) {
  const tokenDiag = {
    present: !!env.HUBSPOT_TOKEN,
    length: env.HUBSPOT_TOKEN ? env.HUBSPOT_TOKEN.length : 0,
    prefix: env.HUBSPOT_TOKEN ? env.HUBSPOT_TOKEN.slice(0, 12) : null,
    suffix: env.HUBSPOT_TOKEN ? env.HUBSPOT_TOKEN.slice(-4) : null,
  };
  try {
    const portalId = await getPortalId(env);
    const account = await hubspotFetch("/account-info/v3/details", env).catch((e) => ({ __error: e.message, __status: e.status }));
    return { connected: true, portalId, accountName: account?.companyName || null, account, tokenDiag };
  } catch (e) {
    return { connected: false, error: e.message, tokenDiag };
  }
}
